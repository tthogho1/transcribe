...
class SparseVectorGenerator:
    ...
    def __init__(
        ...
        min_df: int = 1,  # Changed from 2 to 1
        max_df: float = 0.95,
    ):
        """
        ...
        print(f"✅ Initialized TF-IDF vectorizer parameters (max_features={max_features})")
    ...
    def _create_vectorizer(self, num_docs: int) -> TfidfVectorizer:
        ...
        adjusted_min_df = min(self.min_df, max(1, num_docs // 10))  # At most 10% of docs
        ...
        if adjusted_min_df > max_df_count:
            adjusted_min_df = 1
            adjusted_max_df = 1.0  # Include all documents
        
        print(f"📊 TF-IDF params: docs={num_docs}, min_df={adjusted_min_df}, max_df={adjusted_max_df}")
